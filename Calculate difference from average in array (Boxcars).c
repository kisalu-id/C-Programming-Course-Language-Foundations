...
int main (void) {
    ...
    for (i=0; i<numCars; i++) {
        ...
        array[i] = weightOfCar;
        weightOfAll = weightOfAll + weightOfCar;
    }
        double dnumCars = (double)numCars; //turn int tp double to count
        double medWeightCar = weightOfAll/dnumCars;
        /* just of me to check:
        printf("Num of cars is %d\n", numCars);
        printf("Med weight of car is %.1lf\n", medWeightCar); */
        //creating a variable just to deal with; if weightOfCar = medWeightCar
        double equal = 0.0;
        i = 0;
        for (i=0; i<numCars; i++) {   /* had to use different loop bc i need medWeightCar to have a final value*/
             double result = 0.0;
            if (array [i] != medWeightCar) {
                result = medWeightCar - array [i];
                printf("%.1lf\n", result);
            } else {
        printf("%.1lf\n", equal);
            }
